// Please retain this next 10 lines
//This softwre is FREE!!  Open Source software, do what you want with it, open to the community, etc.....
...
#include <stdio.h>// for output Functions including messagebox
...
public:
	...
	{
		return "Performs a non-linear brightening by using a Gudermannian Sigmoid function.  The Sigmoid a S-Curve and performs a simular function to a Gaussian brightening without having to use a CDF array.  This way we can use floating point values and not have to use lookup tables.";
	}
	...
	int GetUIParameters (UIParameters* pParameters)
	{
		...
		//AddParameter( PARAM_WHITEBALANCE ,"Hyperbolic Tangent|Hyperbolic Sine|ArcTan|Error Function|Gudermannian|Algebraic Sigmoid|ArcTan 2|Algebraic Sigmoid 2|Sigmoid 2|Nine|ten", 0, 0, 10, TYPE_ONEOFMANY, 0);
		AddParameter( PARAM_GAIN ,"Gain", .65, 0.1, 1.5, TYPE_SLIDER, 0.0);
		AddParameter( PARAM_CUTOFF ,"Histogram Shift", 0.0, -0.5, 0.5, TYPE_SLIDER, 0.0);
	...
	virtual void Process_Data (BYTE* pBGRA_out,BYTE* pBGRA_in, int nWidth, int nHeight, UIParameters* pParameters)
	{
		...
		// This routine will seem complex, but only because  we are using different appoaches to white balance
		// all of the routines have to gather various stats about the image.

		//List of Parameters
		//int nWhiteBalance = (int)GetValue(PARAM_WHITEBALANCE);
		double nGain = (float)GetValue(PARAM_GAIN);
		double nCutoff = (float)GetValue(PARAM_CUTOFF);

		double f_colorspace = 255.0;//used for floats, this is the color space 0-255

		double* output = new double[nWidth * nHeight * 4];//working in the 0-1 range
		double* temp = new double[nWidth * nHeight * 4];
		...
		for (int x = 0; x < nWidth; x++)
		{
			for (int y = 0; y < nHeight; y++)
			{
					...
					double red = pBGRA_in[nIdx + CHANNEL_R] / f_colorspace;
					double green = pBGRA_in[nIdx + CHANNEL_G] / f_colorspace;
					double blue = pBGRA_in[nIdx + CHANNEL_B] / f_colorspace;
...
//		if (nWhiteBalance == 4)
		...
		//error function OK
		//1.14 as default
		//double rr = sqrtf(pi) / 2.0f * (gain * (red + cutoff));
		//double gg = sqrtf(pi) / 2.0f * (gain * (green + cutoff));
		//double bb = sqrtf(pi) / 2.0f * (gain * (blue + cutoff));

#pragma region			//restretch image back from 0-1
		...
		for (int x = 0; x< nWidth; x++)
		{
			for (int y = 0; y< nHeight; y++)
			{
					...
					//int nR = temp [nIdx + CHANNEL_R] * f_colorspace;
					//int nG = temp [nIdx + CHANNEL_G] * f_colorspace;
					//int nB = temp [nIdx + CHANNEL_B] * f_colorspace;

					int nR = (output [nIdx + CHANNEL_R] * f_colorspace);
					int nG = (output [nIdx + CHANNEL_G] * f_colorspace);
					int nB = (output [nIdx + CHANNEL_B] * f_colorspace);
...
#pragma endregion
		...
		delete [] output;//delete the array
...
extern "C" __declspec(dllexport) char* GetPluginName()
{
	...
	return "Andy's Sigmoid Brighten";
...
//	ATTENTION
	...
	return "com.lumafilters.Sigmoid";
...
/*
					...
					,
					radius, size, x, y, i, redKernelSlider, redKernelCenter, RcolorDist );MessageBox(NULL,sBuffer1,"Variable Info", MB_OK);//
*/